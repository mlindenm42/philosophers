...
/*   Updated: 2023/07/28 06:17:19 by mlindenm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_ps(void)
{
	...
	while (i < get_d()->nb_of_ps)
	{
		...
		% get_d()->nb_of_ps];
		// printf ("Philo: %d  left fork: %d  right fork: %d\n", d()->ps[i]->number, d()->ps[i]->left_fork->number, d()->ps[i]->right_fork->number);
		if (get_d()->ps[i]->left_fork->number == get_d()->ps[i]->right_fork->number)
			printf("FAIL\n");
...
void	init(int argc, char *argv[])
{
	...
	if (pthread_mutex_init(&(get_d()->m_eat), NULL) != 0)
		error("Failed to initialize mutex for eat %d\n");
